...
const Detail = async ({ params }: { params: { id: number } }) => {
  const res = await fetch(`https://localhost:7151/api/Product/${params.id}`, {
    cache: "no-store",
  });
  ...
  const breadcrumbPaths = [
    ...
    {
      url: `/medicine/${
        product?.prescription ? "prescription" : "nonprescription"
      }`,
      label: product?.prescription ? "Thuốc kê đơn" : "Thuốc không kê đơn",
    },
  ];
